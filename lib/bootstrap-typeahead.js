...
function ($) {
    ...
    Typeahead.prototype = {
        ...
        renderAddItem: function(){
            var that = this;
            if(!this.options.showAddValue) { return; }

            var item = $("<li class='unselect'></li>").append($("<a></a>").text("Would you like to add " + this.$element.val() + "?"));
            that.$menu.html(item);
            that.show();
            return that;
        },
        ...
        add: function() {
            if(this.type !== 'undefined' && !this.$menu.find('.unselect').length){
                ...
                if(clone.find('input').length == 1){
                    item.append(clone.find('input').val($selectedItem.data('value')));
                }
                ...
                this.options.resultAdd(item.data('value'),item.data('text'));
            }
        },

        showModal: function(){
            if(this.options.showAddValue && this.$menu.find('.unselect').length==1){
                this.options.popupAdd(this.$element.val());
            }
        },
